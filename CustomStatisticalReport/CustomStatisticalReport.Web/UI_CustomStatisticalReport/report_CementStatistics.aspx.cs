...
{
    public partial class report_CementStatistics : WebStyleBaseForEnergy.webStyleBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (m_FunctionName == "ExcelStream")
                {
                    ...
                    DayConsumptionReportService.ExportExcelFile("xls", m_Parameter2 + "水泥电耗统计报表.xls", m_ExportTable);
                }
            }
        }
        ...
        public static string GetReportData(string startDate, string endDate)
        {
            ...
            DataTable dt = CementStatisticsService.GetClinkerStatisticsData(levelCodes.ToArray(), startDate, endDate);
        ...
        public static string GetHtml()
        {
            List<string> oganizationIds = WebStyleBaseForEnergy.webStyleBase.GetDataValidIdGroup("ProductionOrganization");
            IList<string> levelCodes = WebUserControls.Service.OrganizationSelector.OrganisationTree.GetOrganisationLevelCodeById(oganizationIds);
            string htmlStr = CementStatisticsService.CreatHtml(levelCodes.ToArray());
            return htmlStr;
        }
        [WebMethod]
        public static List<MergesValueClass> GetMerges()
        {
            return CementStatisticsService.GetFormatInfo();
        }
    }
}