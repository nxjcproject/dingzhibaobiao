...
function initPageAuthority() {
    $.ajax({
        type: "POST",
        url: "report_BalanceStatistics.aspx/AuthorityControl",
        data: "",
...
function LoadData() {
    ...
    //按照抄表周期
    if ("MeterReading" == StatisticalType) {
        if (null == nowSelectRow || null == beforeSelectRow || "" == OrganizationId) {
            $.messager.alert("提示", "请选择组织机构和抄表时间");
            return;
        }
        else {
            //开始时间和结束时间按照选择的抄表时间为准
            var t_dateArray = beforeSelectRow.TimeStamp.split("-");
            startDate = new Date();
            startDate.setFullYear(t_dateArray[0], t_dateArray[1] - 1, t_dateArray[2]);
            startDate.setDate(startDate.getDate() + 1);//不包括上次统计那一天
            startDate = startDate.getFullYear() + "-" + (startDate.getMonth() + 1) + "-" + startDate.getDate();
            endDate = nowSelectRow.TimeStamp;
        }
    }
    else if ("Month" == StatisticalType) { //安照自然月统计
        if (null == nowSelectRow || "" == OrganizationId) {
            $.messager.alert("提示", "必须选择组织机构和本次抄表时间");
            return;
        }
        else {
            //统计时间为本次抄表所在月份
            var selectDate = nowSelectRow.TimeStamp;
            var t_dateArray = selectDate.split("-");
            startDate = new Date();
            startDate.setFullYear(t_dateArray[0], t_dateArray[1] - 1, 1);//统计月一号
            startDate = startDate.getFullYear() + "-" + (startDate.getMonth() + 1) + "-" + startDate.getDate();
            endDate = new Date();
            endDate.setFullYear(t_dateArray[0], t_dateArray[1], 1);
            endDate.setDate(endDate.getDate() - 1);//统计月最后一天
            endDate = endDate.getFullYear() + "-" + (endDate.getMonth() + 1) + "-" + endDate.getDate();
        }
    } else {
        $.messager.alert("提示", "请选择统计类型");
        return;
    }
    ...
    $.ajax({
        ...
        data: '{organizationId: "' + OrganizationId + '", startDate: "' + startDate + '", endDate: "' + endDate + '",statisticalType:"' + StatisticalType + '"}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (msg) {
            dataSet = msg.d;
...
function ExportFileFun() {
    ...
    form.attr('action', "report_BalanceStatistics.aspx");
